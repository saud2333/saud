...
CODED and KFAS are monitored; existing Kuwait University sources are retained. KGBC, KISR and SACGC are retired: their sources are disabled and their opportunities are unpublished on every full bot run. Explicit adapters cover KFAS local programs and CODED's four current public bootcamp pages, while a complete JSON-LD course/event adapter can publish other supported announcements. For CODED, the bot checks both the course page and its exact official application page; the published deadline matches CODED's visible countdown target (the cohort start date at 00:00 Asia/Kuwait). A checked source does **not** mean that every announcement on it can be parsed. Changed templates, missing registration deadlines, inquiry-only pages, unsupported eligibility/fee data and image-only announcements are held or recorded as unsupported, never filled with invented facts. Registration buttons lead to the specific official registration destination.
