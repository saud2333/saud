  ...
  const response = await fetchImpl("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({ model, store: false,
      instructions: [
        ...
        "For every populated evidence field, copy a short exact source quote supporting that field. For image text, supply the corresponding image_url and transcribe the quote. Do not invent evidence. Registration evidence must cover open status and deadline. Schedule evidence must support both start and end. Ages and fees may be null without blocking an otherwise valid announcement. Never infer numeric ages from being a student or employee. Preserve officially stated eligibility and target audience in description_ar. A published minimum-only or maximum-only age keeps the other bound null. When both numeric ages are unpublished set age_label exactly to غير معلن من الجهة and omit age evidence. When fees are unpublished set price_kwd to null and omit price evidence; never call it free.",
      ].join(" "),
