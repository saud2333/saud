...
export async function fetchOfficialPage(url, source, fetchImpl = fetch) {
  ...
  for (let hop = 0; hop < 5; hop++) {
    ...
    return { ...htmlDocument(await boundedText(response), destination, source), requestedUrl: url };
  }
  throw new Error("Too many official page redirects");
}

export async function withImageHashes(document, source, fetchImpl = fetch) {
  const imageHashes = await Promise.all(document.images.map(async (url) => {
    ...
    if (!officialUrl(url, source) && !socialImage) throw new Error("Untrusted image source");
...
export function htmlDocument(html, url, source) {
  const body = html.replace(/<(style|nav|header|footer|noscript)\b[^>]*>[\s\S]*?<\/\1>/gi, " ")
  ...
  for (const match of body.matchAll(/<a\b[^>]*href\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    try {
      const href = new URL(decodeText(match[1]), url).href;
      if (!officialUrl(href, source)) continue;
      links.push(href);
      if (/دور|ورش|تدريب|تسجيل|برنامج|course|workshop|training|register|apply|offer|event|program|FormDetails/i.test(href + " " + textOnly(match[2])) && !/\.(pdf|zip|docx?)(?:[?#]|$)/i.test(href)) candidates.push(href);
    } catch { /* Ignore malformed links. */ }
  }
  for (const match of body.matchAll(/<img\b[^>]*(?:src|data-src)\s*=\s*["']([^"']+)["'][^>]*>/gi)) {
    try {
      const image = new URL(decodeText(match[1]), url).href;
      if (officialUrl(image, source) && !/logo|icon|avatar|flag/i.test(image)) images.push(image);
  ...
  for (const match of html.matchAll(/<meta\b[^>]*>/gi)) {
    ...
    if (attributes.property === "og:image" && attributes.content) {
      try { const image = new URL(attributes.content, url).href; if (officialUrl(image, source) && !/logo|icon/i.test(image)) images.push(image); } catch { /* malformed metadata */ }
    }
  }
  for (const match of body.matchAll(/"(url|image)"\s*:\s*"([^"\n]+)"/g)) {
    try {
      const value = new URL(decodeText(match[2]), url).href;
      if (officialUrl(value, source)) (match[1] === "image" ? images : links).push(value);
    } catch { /* malformed structured URL */ }
  }
  return { url, text: textOnly(body).slice(0, 30_000), links: [...new Set([url, ...links])], images: [...new Set(images)].slice(0, 4), candidates: [...new Set(candidates)], channel: "website" };
}
